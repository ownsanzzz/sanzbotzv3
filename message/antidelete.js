...
${type === 'conversation' ? '*Text :* '+chats : ''}`
        if (type === 'conversation'){
            ikyy.reply(from, tmxt, msg)
        } else if (type === 'stickerMessage' || type === 'imageMessage' || type === 'videoMessage' || type === 'audioMessage' || type === 'documentMessage'){
            let media = await ikyy.downloadMediaMessage(msg)
            ikyy.reply(from, tmxt, msg)
            ikyy.sendMessage(from, media, type, options)
        } else {
            ikyy.reply(from, tmxt, msg)
            ikyy.sendMessage(from, msgOpt, type, options)
        }
    } catch (err) {
